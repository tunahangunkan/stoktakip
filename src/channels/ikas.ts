...
export const ikasConnector: ChannelConnector = {
  ...
  async pushStock(items: StockPushItem[]): Promise<PushResult> {
    ...
    // bulkUpdateProductStock: her kalem { productId? variantId stockCount stockLocationId }
    // İkas'ta stok variant seviyesinde tutulur; channelRef = variantId kabul ediyoruz.
    const mutation = `
      mutation BulkUpdate($input: [BulkUpdateProductStockInput!]!) {
        bulkUpdateProductStock(input: $input)
      }`;
    const input = items.map((it) => ({
      variantId: it.channelRef,
    ...
    try {
      ...
      // İkas senkron cevap verir; ayrı doğrulama gerekmez.
      return { ok: true };
    } catch (e) {
